...
class LegaldocProcessor(DataProcessor):
  """Processor for the LegalDoc data set."""
  ...
  def get_labels(self):
    """See base class."""
    return ["Violation", "family", "labor", "contract", "criminal"]
...
#       # Only the test set has a header
      ...
      guid = "%s-%s" % (set_type, i)
      if set_type == "test":
        text_a = tokenization.convert_to_unicode(line[1]) # line[1] for deepcut texts
        label = "family"
      else:
        text_a = tokenization.convert_to_unicode(line[0]) # line[1] for deepcut texts
        label = tokenization.convert_to_unicode(line[1]) # 'destination' column
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=None, label=label))
    return examples
...
def main(_):
  ...
  processors = {
      ...
      "legaldoc": LegaldocProcessor # Modification: Add LegaldocProcessor to processors
  }
  ...
  label_list = processor.get_labels()
   ...
   # Modification: Add " or task_name == 'legaldoc' " ######################################
  if (task_name == 'xnli' and FLAGS.xnli_language == 'th') or task_name == 'wongnai' or task_name == 'legaldoc':
  ...
  if FLAGS.do_export:
    ...
    estimator.export_savedmodel(os.path.join(FLAGS.output_dir, 'saved_model'), serving_input_receiver_fn)
